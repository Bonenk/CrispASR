...
**Then the instructive part — a plausible "hardening" that measurement rejected.** `qwen3_tts`'s SEANet+RVQ codec computes moshi's `MimiConv1d._get_extra_padding_for_conv1d` **per strided conv** (ceil framing at each stage), which is more principled than a single global pre-pad — so it seemed obvious that pocket-tts's Mimi VAE encoder "should already do that too." It should not. Ported per-conv into `mimi_encode`, built, diffed: onset jumped to **0.0579** and **`backbone_out0` correlation vs the reference collapsed to 0.18** (near-orthogonal conditioning). The pre-pad already **value-matches** the reference; pocket-tts's Mimi VAE encoder effectively pads the input to a whole latent frame and does NOT use independent per-conv extra padding. Reverted.

**Lessons.** (a) An off-by-one in an autoregressive-latent *encoder* is a **positional** bug — it corrupts the onset via RoPE, it doesn't smear evenly; when the *first frame* is wrong, suspect frame **count/alignment**, not per-element math. (b) The codec is guilty only if force-decoding a known-good latent set through it also fails — otherwise localize upstream. (c) A padding/framing convention is a property of the *specific* codec, not the family — do not assume moshi-per-conv (right for qwen3's codec) transfers to the Mimi VAE encoder; prove it with a diff. (d) **Grade conditioning with a deterministic pre-sampling signal.** `backbone_out0` (the first generated frame's backbone output, before the flow head adds noise) correlates cleanly against the reference; the *audio* onset RMS is confounded when comparing our-vs-reference because the two use different RNG streams (only same-seed our-vs-our deltas are meaningful there). The corr metric said 0.18 unambiguously where the onset RMS alone was suggestive but noisy. (e) The §224 voice-latent disk cache keys on the PCM hash, not the encode algorithm — clear `pocket-voice-*.latents` (or `CRISPASR_POCKET_VOICE_CACHE=0`) after any encoder change or a stale latent set masks it.
