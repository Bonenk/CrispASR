...
| **`kugelaudio`** | KugelAudio-0-Open: 7B Qwen2.5 backbone + 4-layer DiT diffusion head (20-step SDE-DPMSolver++) + acoustic VAE decoder → 24 kHz. 23 languages. MIT. | Pre-encoded voices (`--voice voice.gguf`) | ~5.7 GB Q4_K (`--model-quant q4_k`) / ~17.3 GB F16 via `-m auto` — F16 does not fit a 16 GB GPU |
