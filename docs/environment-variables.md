...
| `CRISPASR_T5_QUANT_ALL` | `1` lets `crispasr-quantize` quantize `shared.embed.*` and `lm_head.*` for T5 models too. Off by default: those two carry the 256K vocabulary and are where the Q4_K loss concentrates (#333). |
