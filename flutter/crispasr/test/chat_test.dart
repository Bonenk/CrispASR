...
void main() {
  ...
  test('an interior NUL in an open argument is rejected by field name', () {
    ...
    expect(
      () => CrispasrChatSession.memoryEstimate('mo\u0000del.gguf',
          libPath: libPath),
      throwsA(isA<ArgumentError>().having((e) => e.name, 'name', 'modelPath')),
    );
  });
  ...
  group('real model', () {
    ...
    test('memoryEstimate covers the weights and scales with the context', () {
      if (modelPath == null || modelPath.isEmpty) return;
      final fileSize = File(modelPath).lengthSync();
      expect(fileSize, greaterThan(0));

      int at(int nCtx) => CrispasrChatSession.memoryEstimate(modelPath,
          params: ChatOpenParams(nCtx: nCtx), libPath: libPath);

      // No nCtx: the model's own trained context sizes the KV term.
      expect(CrispasrChatSession.memoryEstimate(modelPath, libPath: libPath),
      ...
      expect(at4k - at2k, equals(2 * (at2k - at1k)),
    ...
    test('open / generate / reset / close', () async {
      ...
      await session.generate(
        const [ChatMessage(role: 'user', content: 'Name a colour.')],
        params: _greedy(16),
      );
      session.reset();
      final afterReset = await session.generate(messages, params: _greedy(16));
      expect(afterReset, equals(reply),
          reason: 'a reset session re-prefills and reproduces the reply');
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('countTokens is positive, monotone, and counts the empty prompt',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      expect(two, lessThan(session.nCtx),
    ...
    test('stop sequences truncate before the match', () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();

      final full =
          await session.generate(_countingMessages, params: _greedy(64));
      ...
      expect(full.contains('5'), isTrue,
          reason: 'the unstopped reply must contain the stop string: $full');
      expect(full, equals('1\n2\n3\n4\n5\n6\n7\n8\n'));
      session.reset();

      final stopped = await session.generate(_countingMessages,
          params: _greedy(64, stop: ['5']));
      expect(stopped.contains('5'), isFalse,
      ...
      session.reset();
      final noStops = await session.generate(_countingMessages,
          params: _greedy(64, stop: const []));
      expect(noStops, equals(full));
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('stop sequences stop at the earliest match in the output', () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();

      final full =
          await session.generate(_countingMessages, params: _greedy(64));
      expect(full.contains('4') && full.contains('7'), isTrue,
    ...
    test('the abort predicate returns true to continue, false to abort',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      expect(continueCalls, greaterThan(0),
    ...
    test('an abort stops early and leaves the session reusable without reset',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      expect(seen, lessThan(fullCalls),
    ...
    test('a stop list mutated from the predicate is freed from the snapshot',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      session.reset();
      expect(await session.generate(_countingMessages, params: _greedy(24)),
          equals('1\n2\n3\n4\n5\n6\n7\n8\n'));
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('an interior NUL is rejected by field name, never truncated',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      expect(
        () => session.countTokens(
            const [ChatMessage(role: 'us\u0000er', content: 'hi')]),
        throwsA(isA<ArgumentError>()
            .having((e) => e.name, 'name', 'messages[0].role')),
      );
      expect(
        () => session.countTokens(
            const [ChatMessage(role: 'user', content: 'a\u0000b')]),
        throwsA(isA<ArgumentError>()
            .having((e) => e.name, 'name', 'messages[0].content')),
      );
      await expectLater(
        session.generate(_countingMessages,
            params: const ChatGenerateParams(stop: ['ok', 'a\u0000b'])),
        throwsA(isA<ArgumentError>().having((e) => e.name, 'name', 'stop[1]')),
      );
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('a partially-filled generate params object decodes as the ABI does',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      final fromPartial = await session.generate(_countingMessages,
          params: const ChatGenerateParams(maxTokens: 24));
      expect(fromPartial, isNotEmpty);
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('a reply cut mid-character is returned, not thrown', () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      expect(cut, equals('\u{1FABF}\uFFFD'),
          reason: 'the incomplete trailing sequence becomes U+FFFD');
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('close frees once, is idempotent, and a closed session throws',
        () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
    ...
    test('a libPath override frees through the library it named', () {
      if (modelPath == null || modelPath.isEmpty) return;
      ...
      final primaryClose = DynamicLibrary.open(libPath)
          .lookup<NativeFinalizerFunction>('crispasr_chat_close');
      final copyClose = DynamicLibrary.open(copyPath)
          .lookup<NativeFinalizerFunction>('crispasr_chat_close');
      // Control: without two distinct addresses every comparison below would
      // hold no matter which library a session picked.
      expect(copyClose.address, isNot(equals(primaryClose.address)),
          reason: 'the copy must load as an independent image');

      const tiny = ChatOpenParams(
          nCtx: 256, nBatch: 64, nUbatch: 64, nGpuLayers: 0, useMmap: true);

      final viaCopy =
          CrispasrChatSession.open(modelPath, params: tiny, libPath: copyPath);
      addTearDown(viaCopy.close);
      expect(viaCopy.closeFunctionAddress, equals(copyClose.address),
          reason: 'the override names the library that must free the session');
      expect(viaCopy.closeFunctionAddress, isNot(equals(primaryClose.address)));

      final viaPrimary =
          CrispasrChatSession.open(modelPath, params: tiny, libPath: libPath);
      addTearDown(viaPrimary.close);
      expect(viaPrimary.closeFunctionAddress, equals(primaryClose.address));
    }, timeout: const Timeout(Duration(minutes: 3)));

    test('a session dropped without close is freed on garbage collection', () {
      if (modelPath == null || modelPath.isEmpty) return;
      ...
      const heapResident = ChatOpenParams(
          nCtx: 256, nBatch: 64, nUbatch: 64, nGpuLayers: 0, useMmap: false);

      // Open a session and drop the only reference to it, without closing.
      void openAndDrop() {
        final s = CrispasrChatSession.open(modelPath,
            params: heapResident, libPath: libPath);
      ...
      expect(peak - start, greaterThan(modelBytes),
      ...
      // clear of both.
      expect(growth, lessThan(4 * modelBytes),
    ...
    test('prefillOnly prefills without generating', () async {
      if (modelPath == null || modelPath.isEmpty) return;
      final session = openSession();
      ...
      final quiet = await session.generate(
        ...
        params: const ChatGenerateParams(
            maxTokens: 64, temperature: 0.0, prefillOnly: true),
      );
