...
| ~~P0 relpos~~ → **P0 hifigan (genuine compute)** | melotts / piper | relpos is a MEASURED DUD (`2026-07-11`): already GEMM'd (`melotts_use_scalar()` gate) and only **~1.5% of synthesis** (text_encoder 190 ms of 12.3 s). **hifigan_decode is 73–92%** and is **genuine GPU compute** (~4× realtime, stable ~8.4 s across 3 warm runs; NOT JIT-bound — the `Apple_M1.archive` works, 0 "add-to-archive skipped", the "compiling pipeline" lines are an unconditional DEBUG log at `ggml-metal-device.m:410` that fires even on an archive hit). No cheap lever — needs a real vocoder-graph campaign (ConvT upsample + MRF resblocks; cf. CV3 HiFT FASTCONV = measured-not-worth-it). Teardown assert FIXED (`119ec75a`, RAII dtor). | HiFi-GAN = genuine compute, big effort |
